        ...
        let reader_handle = tokio::spawn(async move {
            while let Some(msg) = ws_rx.next().await {
                                ...
                                if raw_tx_clone.receiver_count() > 0 {
                                    let session_id =
                                        serde_json::from_str::<serde_json::Value>(&msg)
                                            .ok()
                                            .and_then(|v| {
                                                v.get("sessionId")?.as_str().map(String::from)
                                            });
                ...
                if let Some(id) = parsed.id {
                    ...
                    // command has subscribed to the typed event stream and no
                    // private session route claims the event.
                    let has_private_route = parsed.session_id.as_deref().is_some_and(|sid| {
                        private_clone
                            .lock()
                            .unwrap_or_else(|e| e.into_inner())
                            .contains_key(sid)
                    });
                    if event_tx_clone.receiver_count() > 0 || has_private_route {
                        let event = CdpEvent {
                            method: method.clone(),
                        ...
                        let routed = event.session_id.as_deref().is_some_and(|sid| {
                            let mut routes =
                                private_clone.lock().unwrap_or_else(|e| e.into_inner());
