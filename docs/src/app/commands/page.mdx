...
Commands for a session are serialized. If another browser operation is active, a concurrent request returns a bounded JSON error with <code>code: "session_busy"</code> instead of waiting in a daemon queue. Retry it after the active operation finishes. `close` remains available as a recovery action: it can cancel an unresponsive operation, attempts best-effort transactional persistence within a bounded shutdown window, and force-cleans only browser resources owned by agent-browser.
